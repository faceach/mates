...
namespace Microsoft.ProjectOxford.Facial
{
    using System;
    ...
    public class FaceServiceClient : IFaceServiceClient
    {
        ...
        public async Task<Facial[]> DetectAsync(string url, bool analyzesFacialLandmarks = false, bool analyzesAge = false, bool analyzesGender = false, bool analyzesHeadPose = false)
        {
            ...
            request.Headers["ms-retain-telemetry-image"] = "False";

            dynamic requestBody = new ExpandoObject();
            requestBody.url = url;

            return await this.SendAsync<ExpandoObject, Facial[]>("POST", requestBody, request);
        }
        ...
        /// <param name="analyzesGender">If set to <c>true</c> [analyzes gender].</param>
        /// <param name="analyzesHeadPose">If set to <c>true</c> [analyzes head pose].</param>
        /// <param name="retainImage">If set to <c>true</c> [api will store images].</param>
        /// <returns>The detected faces.</returns>
        public async Task<Facial[]> DetectAsync(Stream imageStream, bool analyzesFacialLandmarks = false, bool analyzesAge = false, bool analyzesGender = false, bool analyzesHeadPose = false, bool retainImage = false)
        {
            var requestUrl = string.Format(
                "{0}/{1}?analyzesFacialLandmarks={2}&analyzesAge={3}&analyzesGender={4}&analyzesHeadPose={5}&{6}={7}",
                ServiceHost,
            ...
            request.Headers["ms-retain-telemetry-image"] = retainImage ? "True" : "False";

            return await this.SendAsync<Stream, Facial[]>("POST", imageStream, request);
        }
